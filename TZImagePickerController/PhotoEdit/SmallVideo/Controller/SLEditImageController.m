...
- (UIButton *)cancleEditBtn {
    if (_cancleEditBtn == nil) {
        _cancleEditBtn = [[UIButton alloc] initWithFrame:CGRectMake(15, isIphoneX?54:30, 40, 30)];
        _cancleEditBtn.hidden = YES;
        _cancleEditBtn.backgroundColor = [UIColor colorWithRed:50/255.0 green:50/255.0 blue:50/255.0 alpha:1];
        _cancleEditBtn.layer.cornerRadius = 4;
...
- (SLEditMenuView *)editMenuView {
    if (!_editMenuView) {
        ...
        _editMenuView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
        ...
        [_editMenuView selectMenuWithIndex:1];
    }
...
- (void)doneEditBtnClicked:(id)sender {
    ...
    switch (self.type) {
        ...
        {
            [self dismissViewControllerAnimated:NO completion:^{
                self.didFinishEditPhoto(self.image);
            }];
        }
            break;
        case DoneTypeAlert:
        {
            ...
            UIAlertAction *action = [UIAlertAction actionWithTitle:@"转发" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            UIAlertAction *sava = [UIAlertAction actionWithTitle:@"保存图片" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [self savePhoto];
            }];
            
            UIAlertAction *cancel = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil];
            [alert addAction:action];
            [alert addAction:sava];
